					...
					<div class="item back-color">
						...
						<div class="con">
							...
							<el-form-item label="限制规则">
									...
									<el-card class="box-card" v-show="limitedRule">
										<el-row>
											<label>周期类型：</label>
											<el-radio-group v-model="limitedRules.period">
										...
										<el-row>
											<label>参与次数：</label>
											<el-input-number v-model="limitedRules.count"  :min="1" size="small"></el-input-number>
										</el-row>
										<el-row>
											<label>总&nbsp;&nbsp;限&nbsp;&nbsp;量：</label>
											<el-input-number v-model="limitedRules.totalAmount"  :min="0" size="small"></el-input-number>
											<span style="color:red;margin-left:10px;">为0时不限总量</span>
										</el-row>
									</el-card>
							</el-form-item>
						</div>
			...
			submitForm(formName) {
				...
				this.$refs[formName].validate((valid) => {
					if(valid) {
						...
						if(this.limitedRule) {
							var rs = {}, rules =  _that.limitedRules;
							rs.period = rules.period;
							rs.count = rules.count;
							rs.totalAmount = rules.totalAmount == 0 ? "" : rules.totalAmount;
							 _that.form.limitedRules = JSON.stringify(rs);
						} else {
							delete _that.form.limitedRules;
						}
