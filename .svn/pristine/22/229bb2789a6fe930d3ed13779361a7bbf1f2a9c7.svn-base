	...
	<div class="prompt-prod">
		<div class="item" v-for="p in prodData">
			...
			<div class="prod-info">
				...
				<p class="inventory" v-if="p.totalAmount">限购：{{p.limitBuyCount}}{{p.skuUnit}}</p>
				<p class="p-info">
					...
					<add-cart v-if="activityStart == 1 && ((p.totalAmount && p.surplusAmount)|| (p.totalAmount == 0))" :row="p" :num="p.skuQuantity" class="cart" @changeProdNum="changeProdNum" ></add-cart>
