...
<template>
	<div class="often-category-prod">
		<div class="c-item" v-for="(c,ck) in categoryProd">
			...
			<div class="prod-list" :class="!editorStatus ? 'editor' : ''">
				<div class="prod-item" v-for="(p, pk) in c.collectProductItemBOList">
					...
					<div class="prod-wrap">
						<add-cart :row="p" :num="p.num" class="cart" @changeProdNum="changeProdNum" v-if="p.offerPrice"></add-cart>
						<span class="no-price" v-else>未报价</span>
						<div class="prod-info">
		...
		<section class="event-all" v-if="!editorStatus" v-show="footerShow">
			<div class="con">
...
<script>
	...
	export default {
		...
		props: ["categoryProd", "editorStatus", "footerShow"],
		components: {
			cartParabola,
			addCart
		},
