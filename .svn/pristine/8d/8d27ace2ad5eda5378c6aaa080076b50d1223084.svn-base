		...
		<article class="register">
			<scroll 
				...
				<article class="register_fooler">
					<a href="javascript:void(0)" class="btn disabled" v-if="nextDisabled">下一步</a>
					<a href="javascript:void(0)" class="btn"  id="btn" @click="changeStatus" v-else>{{fromStatus==1?'下一步':'提交信息'}}</a>
					<p>已有账号，
			...
			<city-agent ref="cityselect" @selectAgent="selectAgent" :locationData="locationData"></city-agent>
		</article>
	</div>
</template>
<script>
	...
	import cityAgent from 'components/common/city-agent'
	...
	export default {
		...
		mounted() {
			var regionId = this.$route.query.regionId, _that = this;
			if(regionId) {
				cJs.setCookie("REGION_ID", regionId, 1);
			}
			setTimeout(() => {
				regionId = cJs.getCookie("REGION_ID")
				if(regionId) {
					registerApi.getDefaultMerchant(_that, regionId);
				}
			},100);
		...
		methods: {
			changeStatus(event) {
				...
				if(this.fromStatus == 1) {
					...
					this.registerData.areaId = this.locationData.areaId;
			...
			selectCity() {
				...
				var _that = this;
				setTimeout(function() {
					_that.$refs.cityselect.show = true;
				}, 100);
			...
			selectAgent(item) {
				this.locationData.areaId = item.areaId;
				this.locationData.areaName = item.provinceName + item.cityName + item.areaName;
			}
		}
	}
</script>
