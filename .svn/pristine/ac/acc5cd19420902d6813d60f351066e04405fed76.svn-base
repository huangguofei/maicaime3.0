		...
		<article class="content">
			...
			<article class="form">
				<section class="item">
					...
					<input type="text" class="phone" maxlength="11" v-model="mobile" placeholder="请输入手机号" @blur="verifyMobile" />
				</section>
				<section class="item">
					<img src="../../images/mine_yuangong_code.png" />
					<input type="text" class="code" maxlength="6" v-focus="isfocus" placeholder="输入验证码" v-model="verificationCode" />
			...
			<button type="button" @click="post" :disabled="!isDisabled" :class="{disabled:!isDisabled}">确定</button>
		</article>
	</article>
</template>
...
<script>
	...
	import loginServer from "actionurl/login/login"
	...
	export default {
		...
		methods: {
			...
			verifyMobile(vm) {
				loginServer.verifyMobile(this, () => {
					cJs.message("该手机还未注册，请确认！")
				});
			}
		}
	}
