...
<template>
	<article class="register-form">
		<section class="from-item">
			...
			<input type="tel" name="phone" id="phone" maxlength="11" @focus="scrollView" @blur="verifyMobile" v-model="mobile" ref="mobile" class="text-input" placeholder="请输入手机号码" />
		</section>
		<section class="from-item">
			<img src="../../images/mine_yuangong_code.png" />
			<input type="number" name="code" id="code" maxlength=6 v-model="verificationCode" v-focus="isfocus" @focus="scrollView"  class="text-input"  ref="verificationCode" placeholder="请输入验证码" />
		...
		<section class="from-item" style="position: relative;">
			...
			<input type="password" name="psw" id="psw" :disabled="isDisabled" v-model="password" @focus="scrollView"  class="text-input"  ref="password" placeholder="请输入密码" />
