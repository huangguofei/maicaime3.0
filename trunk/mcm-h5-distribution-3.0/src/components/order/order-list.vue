			...
			<section v-for="(item,key) in orderData" class="padding bk-white s-item">
				<!-- <router-link :to="{path : '/order/detail?id=' + item.deliveryOrderId}" > -->
					...
					<footer class="padding">
						<article class="" v-if="item.statusId==30||item.statusId==20||item.statusId==40">
							<p class="pull-left">取货号：<span>{{item.articleNumber}}</span>
							</p>
							<span class='pull-right footer-status' v-if="item.statusId==20">备货中</span>
							<span class='pull-right footer-btn' @click="sureDelivery(item)" v-else-if="item.statusId==40">确认送达</span>
	...
	export default {
		...
		methods: {
			...
			orderScroll(pos) {
				...
				orderApi.orderDelivery(this,data.orderId);
			}
		},
		filters: {
