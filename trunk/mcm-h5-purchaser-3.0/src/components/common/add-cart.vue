...
<template>
	<span class="cart-wrap " :class="prodNum > 0 ? 'cart-active' : ''">
		...
		<span class="num" v-show="prodNum > 0">
			<input type="number" :value="prodNum.toString().indexOf('.') == -1 ? prodNum : (prodNum * 1).toFixed(1)" @change="changeNum" class="text-input" />
		</span>
		...
		methods : {
			editSkuNum(state) {
				...
				if(state == 1 && num >= 1) {
					...
					if(this.row.limitBuyCount && num >= this.row.limitBuyCount) {
						cJs.message("限购" + this.row.limitBuyCount + "份");
						return;
					}
					++ num;
					this.prodNum = num;
			...
			changeNum(event) {
				...
				if(value == "") {
					...
					error = "数量必须为数字！";
				} else if(this.row.limitBuyCount && value >= this.row.limitBuyCount) {
					error = "限购" + this.row.limitBuyCount + "份";
				} else {
					var ary = value.split(".");
					if(ary.length > 1) {
						var lastVal = ary[1];
