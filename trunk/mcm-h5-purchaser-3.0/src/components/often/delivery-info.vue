...
<template>
	<div class="delivery-info">
		<div class="d-item d-1">
			<div class="address-info" @click="editAddr">
				<p class="contact">
		...
		<dialog-layer ref="dialogTime" @confirm="confirmTime">
			<div class="dev-times">
				<span class="t-item" :class="{active : k==selectIndex, disable : t.past}" v-for="(t, k) in devTimes"  @click="selectDevTime(t, k)">
					{{returnTimeStr(t)}}
				</span>
			</div>
		</dialog-layer>
		...
		methods: {
			setDevTime() {
				...
				var thatD = thatDate.getFullYear() + "/" + (thatDate.getMonth() + 1) + "/" + thatDate.getDate(),
					thatStr =  new Date().getTime();
				this.devTimes.forEach((item) => {
					var type = item.endTimeType, endTime = item.deliveryEndTimeSection;
					if(type == 0) {
						var endDate = thatD + " " + endTime;
						var endStr = new Date(endDate).getTime() - (2 * 60 * 60 * 1000);
						if(thatStr > endStr) {
							item.past = true;
						}
					}
