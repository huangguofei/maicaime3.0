	...
	<div class="page-layer often-index-page">
		...
		<div class="f-con">
			...
			<often-catrgory-prod 
				...
				@delProdClick="delProd"
				:footerShow="footerShow"
				v-if="categoryProd.length > 0"></often-catrgory-prod>

		</div>
		<loading v-if="load"></loading>
		<footer-nar :active="4" v-show="footerShow"></footer-nar>
		<cart-suspend v-if="editorStatus" v-show="footerShow"></cart-suspend>
		<goods-not-data  v-if="categoryProd.length <= 0  && !load"><p>暂无任何食材!</p></goods-not-data>
	</div>
</template>
<script>
	...
	export default{
		...
		methods : {
			...
			categoryInfo(info) {
				...
				this.load = true;
				OFTENACTION.getProdListByCategory(this, categoryId);
			}
		}
	}
</script>
