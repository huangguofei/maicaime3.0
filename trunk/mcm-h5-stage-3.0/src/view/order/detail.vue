<template>
	<article class="page-layer">
		<div class=" order-details padding" :class="{padding_bottom:orderData.statusId!=100}" v-if="orderData.statusText">
			...
			<section class="shop-details gap" v-for="(val,index) in orderData.list">
				...
				<p class="view-more" v-if="val.prods.length>2&&!val.isSelect" @click=lookMore(val)>(收起)</p>
			</section>
			...
			bindCode() {
				...
				if(cJs.isWeiXin()) {
					cJs.wxScanQRCode(1, (result) => {
						...
						orderApi.bingCodeOrder(_that, _that.orderData.deliveryOrderId, resultStr.split("|")[1]);
					});
				} else {
