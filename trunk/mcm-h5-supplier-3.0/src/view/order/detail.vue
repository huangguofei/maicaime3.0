...
<script>
	...
	export default {
		...
		methods: {
			...
			callPhone() {
				try{
					window.android.call(this.orderDetailsInfo.purchaserMobile);
				} catch(e) {
			...
			printOrderInfo() {
				try{
					...
					var params = {orderId: this.orderId, token: userInfo.token};
					window.android.printOrderAsyn(this.orderId, userInfo.token);
				} catch(e) {
					cJs.message("不支持打印功能");
				}
