<template>
	<article class="page-layer add-bankcard">
		...
		<article class="form padding bk-white">
			...
			<section class="item">
				<span>卡类型</span>
				<select class="card-type" v-model="selectBankCardName">
			...
			<section class="item" v-if="selectBankCardName==''">
				<span>银行卡名称</span>
				<input type="text" class="card-num" v-model="autoBankCardName" maxlength="19" />
			</section>
			<section class="item">
				<span>开户行地址</span>
...
<script>
	...
	export default {
		...
		methods: {
			submit() {
				this.bankCardData.bankName = this.selectBankCardName;
				if(!this.selectBankCardName)
					this.bankCardData.bankName = this.autoBankCardName;
				...
				if(!this.bankCardData.bankName) {
					cJs.message("银行卡类型不能为空！");
					return;
				}
